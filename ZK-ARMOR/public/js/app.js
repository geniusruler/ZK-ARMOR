...
import { computeModelHash, extractModelProperties, formatFileSize, generateZKProof, truncateHash } from './crypto-utils.js';
import { walletConnector } from './wallet-connector.js';
// DOM Elements
let connectWalletBtn;
let disconnectBtn;
let registerFileInput;
let registerDropZone;
let registerFileInfo;
let registerSubmitBtn;
let verifyFileInput;
let verifyDropZone;
let verifyFileInfo;
let verifySubmitBtn;
// State
let currentRegisterFile = null;
let currentVerifyFile = null;
let currentModelHash = '';
// Initialize app
...
function initializeElements() {
    ...
    registerFileInput = document.getElementById('registerFileInput');
    registerDropZone = document.getElementById('registerDropZone');
    registerFileInfo = document.getElementById('registerFileInfo');
    registerSubmitBtn = document.getElementById('registerSubmitBtn');
    verifyFileInput = document.getElementById('verifyFileInput');
    verifyDropZone = document.getElementById('verifyDropZone');
...
async function checkWalletInstallation() {
    ...
    if (!hasLace) {
        showNotification('Lace wallet not detected. Please install it from https://www.lace.io', 'warning');
    }
}
...
function handleRegisterFileSelect(e) {
    const input = e.target;
    if (input.files && input.files[0]) {
        processRegisterFile(input.files[0]);
    }
}
function handleRegisterFileDrop(e) {
    e.preventDefault();
...
async function handleRegisterSubmit() {
    ...
    try {
        ...
        if (result.success) {
            showNotification(`Model registered successfully! Transaction: ${truncateHash(result.txHash)}`, 'success');
            // Reset form
            resetRegisterForm();
        }
        else {
...
function updateProgress(barId, textId, progress, action) {
    const bar = document.getElementById(barId);
    const text = document.getElementById(textId);
    if (bar)
        bar.style.width = `${progress}%`;
    if (text)
        text.textContent = `${action}... ${progress}%`;
}
